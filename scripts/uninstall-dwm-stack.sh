...
if [[ $restore -eq 1 ]]; then
  ...
  restore_latest "$HOME/.config/picom"
...
if [[ -L "$HOME/.config/picom" ]]; then
  run_cmd "rm -f '$HOME/.config/picom'"
fi
